...
class Bus:
    """
    Represents a bus and its seats with 12-hour AM/PM schedule support.
    """
    ...
    def _format_datetime(dt_string):
        """Format YYYY-MM-DD HH:MM into a clean 12-hour AM/PM string: 20 Sep, 08:30 AM"""
        try:
    ...
    def departure_display(self):
        return self._format_datetime(self.departure_dt)

    @property
    def arrival_display(self):
        return self._format_datetime(self.arrival_dt)

    def has_departed(self):
        try:
            dep = datetime.strptime(self.departure_dt, "%Y-%m-%d %H:%M")
    ...
    def get_seat(self, seat_number: int):
        if seat_number not in self.seats:
            raise InvalidSeatError(
                f"Seat {seat_number} does not exist on bus {self.bus_id}."
            )
    ...
    def from_dict(cls, data):
        ...
        bus.seats = {
            int(seat_no): Seat.from_dict(seat_data)
            for seat_no, seat_data in data["seats"].items()
        }

        return bus

    # -------------------------------------------------
    # SEAT LAYOUT DISPLAY (CLI)
    # -------------------------------------------------
    ...
    def display_layout(self):
        ...
        print(f"Departure   : {self.departure_display}")
        print(f"Arrival     : {self.arrival_display}")
        print(f"Fare        : Rs.{self.fare:.2f}")
        print("=" * 60)
        print("\n[##] Available   [XX] Reserved\n")

        rows = (self.total_seats + cols - 1) // cols

        for row in range(rows):
            row_text = f"Row {row + 1:02d} : "
            for col in range(cols):
                seat_no = row * cols + col + 1
                if seat_no <= self.total_seats:
                    seat = self.seats[seat_no]
                    tag = "[XX]" if seat.is_reserved else f"[{seat_no:02d}]"
                    row_text += f"{tag:<8}"
            print(row_text)
        print()